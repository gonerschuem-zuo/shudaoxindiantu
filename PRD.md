...
### v1.04.1 (2026-04-09) - UI优化 + 多端同步 ✨

#### UI优化
- 🎨 **全面升级视觉设计**：参考 vO.app 设计语言，实现玻璃态效果
  - 卡片：玻璃态背景 + 24px 圆角 + 三层阴影系统
  - 按钮：光波效果 + 弹性动画 + 改进的按压反馈
  - 轮播：玻璃态导航按钮 + 更精致的指示器
  - 色彩：深墨蓝(#1a1a2e)、金棕色(#c9a96e)、胭脂红(#e85d75)
  - 字体：统一设计系统变量，便于全局调整
- 📱 **移动端体验优化**：
  - 上传区域移至页面底部，不再遮挡卡片
  - 按钮字体缩小 4-8 倍，在手机端更精致
  - 点击卡片直接弹出文件选择（无需滚动到顶部）
  - 固定底部控制栏，方便移动端操作
- ✨ **微交互动画**：
  - 点赞心跳动画
  - 骨架屏加载状态
...
#### 多端同步功能
- ✨ **新增卡片信息云端同步**：
  - 创建 `calligraphy_card_settings` 表存储用户编辑的卡片信息
  - 用户在任何设备上编辑卡片，其他设备登录后自动同步
  - 数据优先级：云端 > 本地（确保使用最新数据）
  - 支持 upsert 操作（有则更新，无则插入）
- ✨ **自动同步触发**：
  - 登录时：自动加载云端卡片设置
  - 注册时：自动加载云端卡片设置
  - 编辑保存时：自动同步到云端
...
- 🔒 **数据安全**：
  - RLS 策略确保用户只能访问自己的数据
  - 云端失败时降级到 localStorage
  - 删除账号时自动清除相关数据

#### 交互优化
- ✨ **简化上传流程**：点击卡片 → 直接弹出文件选择（支持多选）
- ✨ **上传进度提示**：专用 toast 通知，实时显示处理进度
- ✨ **选中提示优化**：始终可见，悬停时有微缩放效果

#### 数据库设计
**新增表**：
- `calligraphy_card_settings` - 卡片设置表
  - 字段：id, user_id, card_id, icon, name, age, style_title, style_desc, updated_at
  - 约束：UNIQUE(user_id, card_id) - 每个用户对每张卡片只有一条记录
  - 字段长度验证：icon ≤ 2字符，其他字段不能为空

**新增函数**：
- `upsert_card_settings()` - 便捷的 upsert 函数

**RLS 策略**：
- 用户只能查看/插入/更新/删除自己的卡片设置

#### 技术实现
**前端架构**：
- 使用 CSS 变量系统（:root）统一管理设计令牌
- 保持所有现有类名和 JS 选择器不变
- 异步函数支持云端同步（async/await）

**数据流**：
```
编辑卡片 → 保存到本地 → 同步到云端 → 其他设备登录 → 自动加载
```
